...
import com.sapient.recipeapp.presentation.model.mapper.IngredientItemMapper
import com.sapient.recipeapp.presentation.model.mapper.InstructionItemMapper
import com.sapient.recipeapp.presentation.model.mapper.RecipeItemMapper
import com.sapient.recipeapp.presentation.model.mapper.StepsItemMapper
...
internal class RecipeListViewModelTest {
    ...
    fun setUp() {
        ...
        val observer = mock<Observer<List<RecipeItem>?>>()
    ...
    fun `when getRecipe returns the data as emptyList`() {
        ...
        `when`(getRecipesUseCase()).thenReturn(flow { emptyList<RecipeItem>() })
    ...
    fun `when getRecipe returns the data`() = runBlocking {
        withContext(Dispatchers.IO) {
            ...
            viewModel.recipesData.observeForever { recipesList ->
                ...
                assertEquals(
                    recipesList,
                    recipeList.map { repoItemMapper.mapToPresentation(recipe) })
            }
        }
    }
    ...
    fun `when insert recipe then verify isFav is true`() {
        ...
        viewModel.addFavourite(repoItemMapper.mapToPresentation(recipe))
    ...
    fun `when remove recipe then verify isFav is false`(){
        ...
        viewModel.removeFavourite(repoItemMapper.mapToPresentation(recipe))
